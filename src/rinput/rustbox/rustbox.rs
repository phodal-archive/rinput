...
pub use style::{RB_BOLD, RB_NORMAL, RB_REVERSE, RB_UNDERLINE, Style};

use crate::rustbox::mouse::Mouse;
use crate::rustbox::keyboard::Key;
...
impl RustBox {
    ...
    pub fn init(opts: InitOptions) -> Result<RustBox, InitError> {
        ...
        let stderr = if opts.buffer_stderr {
            Some(Hold::stderr().map_err(|e| InitError::BufferStderrFailed(e))?)
        } else {
            None
        };
